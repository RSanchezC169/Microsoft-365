...
This script is used to Enable all conditional access policies in the tenant. 
#>
##################################################################################################################################################################
#==============================Beginning of script================================================================================================================
##################################################################################################################################################################
...
Function Enable-AllCAPolicies {
    try {
        ...
        # Loop through each policy and enable it with a progress bar
        ...
        foreach ($policy in $policies) {
            ...
            Write-Progress -Activity "Enabling Conditional Access Policies" `
                -Status "Processing $CurrentPolicy of $TotalPolicies policies..." `
                -PercentComplete $ProgressPercentage

            # Check if the policy is already enabled
            if ($policy.State -eq "Enabled") {
                Write-Log -Message "Policy '$($policy.DisplayName)' is already enabled. Skipping..." -Level "INFO"
                continue
            }

            # Enable the policy
            Write-Log -Message "Enabling policy: $($policy.DisplayName)" -Level "INFO"
            try {
                Update-MgIdentityConditionalAccessPolicy -ConditionalAccessPolicyId $policy.Id -BodyParameter @{state = "Enabled"}
                Write-Log -Message "Policy '$($policy.DisplayName)' has been enabled successfully." -Level "INFO"
            } catch {
                Write-Log -Message "Failed to enable policy '$($policy.DisplayName)'. Error: $($_.Exception.Message)" -Level "ERROR"
            }
        }
        ...
        Write-Log -Message "All conditional access policies have been processed." -Level "INFO"
...
try {
    ...
    # Step 6: Enable conditional access policies
    Write-Log -Message "Enabling Conditional Access policies..." -Level "INFO"
    try {
        Enable-AllCAPolicies
        Write-Log -Message "Conditional Access policies enabled successfully." -Level "INFO"
    } catch {
        Write-Log -Message "Error enabling Conditional Access policies: $($_.Exception.Message)" -Level "ERROR"
        throw
    }
    Clear-Host
