...
This script is used to disable all conditional access policies in the tenant. 
...
Function Disable-AllCAPolicies {
    try {
        ...
        if (-not $policies -or $policies.Count -eq 0) {
            ...
            Write-Log -Message "Checking if security defaults is enabled and disabling if necessary..." -Level "INFO"
            ...
            if ((Get-MgPolicyIdentitySecurityDefaultEnforcementPolicy).IsEnabled -eq $true) {
                ...
                Write-Log -Message "Security defaults are enabled. Disabling now..." -Level "INFO"
                Update-MgPolicyIdentitySecurityDefaultEnforcementPolicy -BodyParameter $params
                Write-Log -Message "Security defaults disabled successfully." -Level "INFO"
            } else {
                Write-Log -Message "Security defaults are not enabled. No changes needed." -Level "INFO"
            }
            return
        }

        # Loop through each policy and disable it with a progress bar
        ...
        foreach ($policy in $policies) {
            ...
            Write-Progress -Activity "Disabling Conditional Access Policies" `
                -Status "Processing $CurrentPolicy of $TotalPolicies policies..." `
                -PercentComplete $ProgressPercentage

            Write-Log -Message "Disabling policy: $($policy.DisplayName)" -Level "INFO"

            try {
                Update-MgIdentityConditionalAccessPolicy -ConditionalAccessPolicyId $policy.Id -BodyParameter @{state = "disabled"}
                Write-Log -Message "Policy '$($policy.DisplayName)' has been disabled successfully." -Level "INFO"
            } catch {
                Write-Log -Message "Failed to disable policy '$($policy.DisplayName)'. Error: $($_.Exception.Message)" -Level "ERROR"
            }
        }
        ...
        Write-Log -Message "All conditional access policies have been disabled. You can re-enable them manually as needed." -Level "INFO"
...
try {
    ...
    # Step 6: Disable conditional access policies and/or security defaults
    Write-Log -Message "Disabling Conditional Access policies and/or security defaults..." -Level "INFO"
    try {
        Disable-AllCAPolicies
        Write-Log -Message "Conditional Access policies and security defaults disabled successfully." -Level "INFO"
    } catch {
        Write-Log -Message "Error disabling Conditional Access policies or security defaults: $($_.Exception.Message)" -Level "ERROR"
        throw
    }
    Clear-Host
    ...
    [System.GC]::Collect()
    [System.GC]::WaitForPendingFinalizers()
    Write-Log -Message "Resources cleaned up successfully." -Level "INFO"

    # Open the log file for review
    Write-Log -Message "Opening log file for review..." -Level "INFO"
