...
<head>
    ...
    <script>
        ...
        function filterLogs() {
            var selectedUser = document.getElementById("userDropdown").value; // Get selected user
            var rows = document.querySelectorAll(".logRow"); // Get all rows with the 'logRow' class

            rows.forEach(function(row) {
                // Show row if the user matches or "All" is selected
...
"@
        ...
        if ($totalLogs -gt 0) {
            foreach ($log in $SignInLogs) {
                ...
                # Add a row for each log with a data-user attribute for filtering
